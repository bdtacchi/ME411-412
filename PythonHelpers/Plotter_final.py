...
colorNum = int(initialStringList[-2]) # Define the current color
colorHolder = colorNum # Define a previous color to change the title of the plot once the color changes

initialTime = int(initialStringList[-3]) # Define the current time
...
while line < samples:
	...
	for i in spec_line_str_list[:-4]: # Make the spectrometer data array. Leave the last for out (integral, time, color, and number of channels)
		if i == '':
			i = '0'
