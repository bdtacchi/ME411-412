...
//    data_matrix(i) = data[i];
  ...
  Serial.print(',');
  Serial.print(timeSpec);
  Serial.print(',');
  Serial.print(timeIntegral);
  Serial.print(',');
...
void loop(){
  initialTime = micros();
  changeColor();
  timeColor = micros() - initialTime;
  initialTime = micros();
  readSpectrometer();
  timeSpec = micros() - initialTime;
  initialTime = micros();
  calcIntLoop();
  timeIntegral = micros() - initialTime;
  initialTime = micros();
  printData();
  timePrint = micros() - initialTime;
  delay(100);  
